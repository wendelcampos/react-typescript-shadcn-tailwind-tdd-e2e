...
const statuses: OrderStatus[] = ['pending', 'processing', 'delivering', 'canceled', 'delivered']
...
export const getOrdersMock = http.get<never, never, GetOrdersResponse>(
    ...
    async ({ request }) => {
        ...
        if(orderId) {
            filteredOrders = filteredOrders.filter(order => order.customerName.includes(orderId))
        }
